@extends('frontend.master')
@section('content')
    <div style="background-image: url(mobile-assets/dept-pharmacy/s2-bg-img-1.webp);">
          <div class="mobile" >
            @include('frontend/components/mobileheader')
                <img style="padding-top: 80px; position: relative;" src="mobile-assets/dept-pharmacy/mobile-pharma.png" alt="">

    <div class="container pb-5 pt-3">

        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              1. Events & Achievements
            </h3>
            <div class="table-responsive">
              <table
                class="overflow-hidden table text-wrap table-bordered border-top mb-5"
              >
                <thead class="text-white" style="background-color: #27467a">
                  <tr>
                    <th class="font-weight-bold text-white">Date of Visit</th>
                    <th class="font-weight-bold text-white">Name</th>
                    <th class="font-weight-bold text-white">Designation</th>
                    <th class="font-weight-bold text-white">Purpose of the visit</th>
                  </tr>
                </thead>

                <tbody class="para1 align-middle" style="background-color: #f9f9f9; text-align: start;">
                  <tr>
                    <td>9/27/2019</td>
                    <td>Mr. Arjuna Vaidya</td>
                    <td>CEO, Dr. Vaidya's (Herbolab India)</td>
                    <td>Interactive Session</td>
                  </tr>
                  <tr>
                    <td>11/20/2019</td>
                    <td>Dr. Jogesh Sarma</td>
                    <td>Professor and HOD, Dept. of Pulmonary Medicine, GMCH.</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/22/2019</td>
                    <td>Dr. Ramu Adela</td>
                    <td>
                      Assistant Professor, Dept. of Pharmacy Practice, NIPER, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/25/2019</td>
                    <td>Dr. AC Kataky</td>
                    <td>
                      Director and Professor, B. Barooah Cancer Hospital, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>12/6/2019</td>
                    <td>Dr. Atanu Bhattacharjee</td>
                    <td>Associate Professor, ADTU, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>12/6/2019</td>
                    <td>Mrs. Chayanika Bordoloi</td>
                    <td>Assistant Professor, NEF College of Pharmacy, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/28/2020</td>
                    <td>Dr. Sanjay K Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/4/2021</td>
                    <td>Dr. VGM Naidu</td>
                    <td>Dean and Associate Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/15/2022</td>
                    <td>Dr. Dipankar Dakua</td>
                    <td>
                      Palliative Care Physician, Assam Cancer Care Foundation,
                      Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>5/6/2022</td>
                    <td>Dr. Krishna Undela</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>7/12/2022</td>
                    <td>Dr. Krishna Undela</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>9/25/2022</td>
                    <td>Dr. Ramu Adela</td>
                    <td>
                      Assistant Professor, Dept. of Pharmacy Practice, NIPER, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>9/25/2022</td>
                    <td>Dr. Banavath Sandeep Kumar Naik</td>
                    <td>
                      Senior Drug Safety Associate, Accenture Services Limited,
                      Banagalore
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/7/2022</td>
                    <td>Dr. Arun Deka</td>
                    <td>
                      Associate Professor, Dept. of Pain &amp; Palliative Medicine,
                      GMCH
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/18/2022</td>
                    <td>Dr. USN Murthy</td>
                    <td>Director, NIPER-G</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/19/2022</td>
                    <td>Dr. T.K. Maity</td>
                    <td>Professor, Jadavpur University, Kolkata</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/20/2022</td>
                    <td>Dr. Dilip Solanki</td>
                    <td>General Manager, Sun Pharma</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/21/2022</td>
                    <td>Dr. Sanjay K Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>2/14/2023</td>
                    <td>Dr. Awanish Mishra</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>3/10/2023</td>
                    <td>Dr. Rajalakshi Devi</td>
                    <td>Professor &amp; HOD, IASST, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>

                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. B.B. Kakoty</td>
                    <td>
                      Professor, Dept. of Pharm. Sciences, Dibrugarh University,
                      Dibrugarh
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. Pankaj Chetia</td>
                    <td>
                      Associate Professor, Dept. of Life Sciences, Dibrugarh
                      University, Dibrugarh
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. Bhanu P. Sahu</td>
                    <td>Professor, Dept. of Pharmacy, GCU, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. Subham Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>9/25/2023</td>
                    <td>Mr. Sudhir Chandra Saha</td>
                    <td>Deputy Director Pharmacy &amp; General Secretary, AAPSA</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>9/25/2023</td>
                    <td>Mr. Rafiquz Zaman</td>
                    <td>Chief Advisor, AAPSA</td>
                    <td></td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. (Dr.) T. Puniyamurty</td>
                    <td>Professor, IIT-G</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. (Dr.) Utpal Bora</td>
                    <td>Professor, IIT-G</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Dr. Pijus K. Sasmal</td>
                    <td>Professor, JNU</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. Arup K. Mukharjee</td>
                    <td>Professor, Makaut, WB</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Dr. Bhanu P. Sahu</td>
                    <td>Professor, Dept. of Pharmacy, GCU, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. (Dr.) Swati Biswas</td>
                    <td>Professor, BITS Pillani</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Prof. (Dr.) Saorav Pal</td>
                    <td>Professor, Ashoka University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Dr. Sanjay K Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Prof. (Dr.) Vidya Avasare)</td>
                    <td>Professor, Ashoka University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Prof. (Dr.) Panna Thapa</td>
                    <td>Professor, Kathmandu University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Dr. K. Gautaman</td>
                    <td>Professor, Govt. College of Pharmacy, Sikkim</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Dr. P. Chattopadhyay</td>
                    <td>Professor, DRL, Tezpur</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Prof. (Dr.) Md Abdur Rashid</td>
                    <td>Professor, University of Dhaka</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. B.B. Kakoty</td>
                    <td>
                      Professor, Dept. of Pharm. Sciences, Dibrugarh University,
                      Dibrugarh
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. Pavan M. More</td>
                    <td>ICT, Mumbai</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. S.C. Gupta</td>
                    <td>Professor, AIIMS, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Pro. Mahesh D. Burande</td>
                    <td>Director, IPER, Pune</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Prof. (Dr.) Prodeep Phukan</td>
                    <td>Professor, Gauhati University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. Bappaditya Gole</td>
                    <td>Professor, Shiv Nadar University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>1/12/2024</td>
                    <td>Swami Ajatananda Ji</td>
                    <td>RamaKrishna Mission Ashrama, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>2/5/2024</td>
                    <td>Dr. Dipankar Dakua</td>
                    <td>
                      Palliative Care Physician, Assam Cancer Care Foundation,
                      Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/29/2024</td>
                    <td>Dr. Ujjal Chakravarty</td>
                    <td>
                      Geeneral and Laproscopic Sergeon, EEN Nursin Home, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>3/6/2024</td>
                    <td>Prof. (Dr.) Hemanta Kr. Sharma</td>
                    <td>Member, PCI</td>
                    <td>Guest Lecture</td>
                  </tr>

                  <tr>
                    <td>3/6/2024</td>
                    <td>Mrs. Sukriti Das</td>
                    <td>Member, PCI</td>
                    <td></td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Harlokesh N Yadav</td>
                    <td>Additional Professor, AIIMS, New Delhi</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Sumalya Sen</td>
                    <td>
                      Assistant Professor, GD Institute of Medical Sciences and
                      Hospital, Durgapur
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Bhabatosh Das</td>
                    <td>Professor, THSTI, Faridabad</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. S.S. Samajdar</td>
                    <td>Independent Clinical Pharmacologist, Kolkata</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Amit Dey</td>
                    <td>Diabetologist, Apollo Sugar Clinic Ltd.</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Alok Modi</td>
                    <td>Senior Diabetologist, Kevalaya Hostpital, Maharastra</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Praveen TK</td>
                    <td>Professor, JSS College of Pharmacy, Ooty</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Sai Balaji A</td>
                    <td>Scientist/Assistant Professor, CSIR-IICT, Hyderabad</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Ashish Kr. Kakkar</td>
                    <td>Associate Professor, PGIMER, Chandigarh</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Bibyajyoti Saikia</td>
                    <td>Assistant Professor, AIIMS, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Payodhi Dhar</td>
                    <td>Founder &amp; President, Kolkata Nurture Foundation</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Sangeeta Yadav</td>
                    <td>
                      Assistant Professor, Dr. B.S. Ambedkar Medical College, Delhi
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Parodeep Dwivedi</td>
                    <td>Additional Professor, AIIMS, Jodhpur</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Ashutosh Kumar</td>
                    <td>Assistant Professor, NIPER, SAS Nagar</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Mukesh Nandave</td>
                    <td>Associate Professor, DPSRU, New Delhi</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Parkash Diwan</td>
                    <td>Advisor, Indian Pharmacological Society</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/10/2024</td>
                    <td>Mr. Sreejit Soman</td>
                    <td>Application Scientist, Zastra Innovations, Bangalore</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/11/2024</td>
                    <td>Dr. Krishna Undela</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>9/25/2024</td>
                    <td>Mr. Purna Sindhu Mudoi</td>
                    <td>Inspector of Drugs, Kamrup (M)</td>
                    <td>Guest Lecture</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>


        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              2. Departmental Research Statistics
            </h3>
            <div class="text-dark">
              <h3 class="headd3 text-dark fw-bold kd-split-text pt-2">
                <a
                  href="https://www.rgu.ac/research-and-development-rsaps-physics.php"
                >
                  Click More</a
                >
              </h3>
            </div>
          </div>
        </div>


        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              3. The Board of Studies (BoS)
            </h3>
            <div class="table-responsive">
              <table
                class="overflow-hidden table text-wrap table-bordered border-top mb-5"
              >
                <thead class="text-white" style="background-color: #27467a">
                  <tr>
                    <th class="text-white">#</th>
                    <th class="text-white">Position in S-BoS</th>
                    <th class="text-white">Name and Designation</th>
                  </tr>
                </thead>
                <tbody class="para1 align-middle" style="background-color: #f9f9f9; text-align: start;">
                  <tr>
                    <td>1</td>
                    <td>Convener (Ex-Officio)- Head of the Department</td>
                    <td>Prof. L.K. Nath, Dean, RSP</td>
                  </tr>
                  <tr>
                    <td>2</td>
                    <td>All Heads of the Departments - Members (Ex-Officio)</td>
                    <td>Prof. Bipul Nath, Principal HoD, RSP</td>
                  </tr>
                  <tr>
                    <td>3</td>
                    <td>Professors of the School Members (Ex-Officio)</td>
                    <td>
                      Prof. Atanu Bhattacharjee, Professor, RSP
                      <hr />
                      Prof. Prithviraj Chakraborty, Professor, RSP
                    </td>
                  </tr>
                  <tr>
                    <td>4</td>
                    <td>External Experts for each Department (Academic)</td>
                    <td>Prof. Bhanu P Sahu, Dean, GIPS, GCU</td>
                  </tr>
                  <tr>
                    <td>5</td>
                    <td>External Experts for each Department (Industry)</td>
                    <td>Mr. Utpal Kalita, Sr. Manager, Ajanta Pharma Ltd</td>
                  </tr>
                  <tr>
                    <td>6</td>
                    <td>
                      Two Faculty members of the School nominated by the Vice
                      Chancellor
                    </td>
                    <td>
                      Dr. Pranabesh Sikdar, Assistant Professor, RSP
                      <hr />
                      Mr. Kamal Deka, Assistant Professor, RSP
                    </td>
                  </tr>
                  <tr>
                    <td>7</td>
                    <td>Registrar, Member Secretary (Ex-officio)</td>
                    <td>Dr. D N Singh, Registrar or his nominee</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>

        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              4. The Departmental Research Committee (DRC)
            </h3>
            <div class="table-responsive">
              <table
                class="overflow-hidden table text-wrap table-bordered border-top mb-5"
              >
                <thead class="text-white" style="background-color: #27467a">
                  <tr>
                    <th class="text-white">#</th>
                    <th class="text-white">Content</th>
                    <th class="text-white">Name of the Member</th>
                    <th class="text-white">Designation</th>
                    <th class="text-white">Designation in the committee</th>
                  </tr>
                </thead>
                <tbody class="para1 align-middle" style="background-color: #f9f9f9; text-align: start;">
                  <tr>
                    <td>1</td>
                    <td>Head of the Department</td>
                    <td>Prof. (Dr.) Prithviraj Chakraborty</td>
                    <td>Professor&amp; Principal/HoD, RSP</td>
                    <td>Chairman</td>
                  </tr>
                  <tr>
                    <td rowspan="2">2</td>
                    <td rowspan="2" class="align-middle">Two Professors</td>
                    <td>Prof. (Dr.) Bipul Nath</td>
                    <td>Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td>Prof. (Dr.) Atanu Bhattacharjee</td>
                    <td>Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td rowspan="2">3</td>
                    <td rowspan="2" class="align-middle">
                      Two Associate Professors – one of them will be the member
                      secretary
                    </td>
                    <td>Dr. Debarupa Dutta Chakraborty</td>
                    <td>Associate Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td>Dr. Pranabesh Sikdar</td>
                    <td>Associate Professor, RSP</td>
                    <td>Member Secretary</td>
                  </tr>
                  <tr>
                    <td rowspan="2">4</td>
                    <td rowspan="2" class="align-middle">
                      Two Assistant Professors holding Ph.D. degree
                    </td>
                    <td>Dr. Ayesha A.K. Choudhury</td>
                    <td>Assistant Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td>Dr. Ayesha A.K. Choudhury</td>
                    <td>Dr. Syed Nazrin Ruhina Rahman</td>
                    <td>Assistant Professor, RSP, Member</td>
                  </tr>
                  <tr>
                    <td rowspan="3">5</td>
                    <td rowspan="3" class="align-middle">
                      Three external members not below the rank of Professors,
                      including members from an allied department, who will be
                      nominated by the URC
                    </td>
                    <td>Prof. (Dr.) Mina Ram Nath</td>
                    <td>Professor, Dept. of Botany, RSLSC</td>
                    <td>External Member</td>
                  </tr>
                  <tr>
                    <td>Prof. (Dr.) Ankur Ganguly</td>
                    <td>Professor &amp; Dean, Academics &amp; i/c Dean, RSIT</td>
                    <td>External Member</td>
                  </tr>
                  <tr>
                    <td>Prof. (Dr.) Kaberi Saikia</td>
                    <td>Professor &amp;Principal, RSN.</td>
                    <td>External Member</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>

        <div>
          <h3
            class="headd3 text-dark fw-bold p-3"
            style="background-color: #c7daf9; color: #ffffff"
          >
            5. Course Structure & Syllabus
          </h3>
          <div class="p-2">
            <h3 class="headd3 text-dark fw-bold kd-split-text pt-2">Syllabus -</h3>
            <div class="row">
              <div class="col-lg-12">
                <div class="accordion para1" id="accordionExample">
                  <div class="accordion-item">
                    <h2 class="accordion-header" id="headingOne">
                      <button
                        class="accordion-button collapsed"
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#collapseOne"
                        aria-expanded="false"
                        aria-controls="collapseOne"
                      >
                    <strong>
                      Under Graduate
                      </strong>
                      </button>
                    </h2>
                    <div
                      id="collapseOne"
                      class="accordion-collapse collapse"
                      aria-labelledby="headingOne"
                      data-bs-parent="#accordionExample"
                    >
                      <div class="accordion-body">
                        <div class="accordion-body">
                          <div class="row">
                            <a
                              href="assets/Departmentalpdf/AY 2021-22 D.Pharmacy.pdf"
                              target="_blank"
                            >
                              <i class="fa fa-file-text px-2"></i> AY 2021-22
                              D.Pharmacy.pdf <i class="fa fa-download"></i>
                            </a>
                            <a
                              href="assets/Departmentalpdf/Old Syllabus B. Pharmacy.pdf"
                              target="_blank"
                            >
                              <i class="fa fa-file-text px-2"></i> Old Syllabus B.
                              Pharmacy.pdf <i class="fa fa-download"></i>
                            </a>
                            <a
                              href="assets/Departmentalpdf/Old Syllabus B_Pharm.pdf"
                              target="_blank"
                            >
                              <i class="fa fa-file-text px-2"></i> Old Syllabus B_Pharm.pdf
                              <i class="fa fa-download"></i>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="accordion-item">
                    <h2 class="accordion-header" id="headingTwo">
                      <button
                        class="accordion-button collapsed"
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#collapseTwo"
                        aria-expanded="false"
                        aria-controls="collapseTwo"
                      >
                    <strong>
                      Post Graduate
                      </strong>
                      </button>
                    </h2>
                    <div
                      id="collapseTwo"
                      class="accordion-collapse collapse"
                      aria-labelledby="headingTwo"
                      data-bs-parent="#accordionExample"
                    >
                      <div class="accordion-body"></div>
                    </div>
                  </div>
                  <div class="accordion-item">
                    <h2 class="accordion-header" id="headingThree">
                      <button
                        class="accordion-button collapsed"
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#collapseThree"
                        aria-expanded="false"
                        aria-controls="collapseThree"
                      >
                    <strong>
                      Doctoral Programme
                      </strong>
                      </button>
                    </h2>
                    <div
                      id="collapseThree"
                      class="accordion-collapse collapse"
                      aria-labelledby="headingThree"
                      data-bs-parent="#accordionExample"
                    >
                      <div class="accordion-body">
                        <a
                          href="doctoral-programme.html"
                          class="para1"
                          target="_blank"
                          >
                        <strong>
                        Click to View...
                      </strong>
                      </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-6"></div>
            </div>
          </div>
        </div>
      </div>
          </div>

          <div class="website">
            @include('frontend/components/aheader')
            <img src="mobile-assets/dept-pharmacy/web-pharmacy.png" alt="">

    <div class="container pb-5 pt-3">

        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              1. Events & Achievements
            </h3>
            <div class="table-responsive">
              <table
                class="overflow-hidden table text-wrap table-bordered border-top mb-5"
              >
                <thead class="text-white" style="background-color: #27467a">
                  <tr>
                    <th class="font-weight-bold text-white">Date of Visit</th>
                    <th class="font-weight-bold text-white">Name</th>
                    <th class="font-weight-bold text-white">Designation</th>
                    <th class="font-weight-bold text-white">Purpose of the visit</th>
                  </tr>
                </thead>

                <tbody class="para1 align-middle" style="background-color: #f9f9f9; text-align: start;">
                  <tr>
                    <td>9/27/2019</td>
                    <td>Mr. Arjuna Vaidya</td>
                    <td>CEO, Dr. Vaidya's (Herbolab India)</td>
                    <td>Interactive Session</td>
                  </tr>
                  <tr>
                    <td>11/20/2019</td>
                    <td>Dr. Jogesh Sarma</td>
                    <td>Professor and HOD, Dept. of Pulmonary Medicine, GMCH.</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/22/2019</td>
                    <td>Dr. Ramu Adela</td>
                    <td>
                      Assistant Professor, Dept. of Pharmacy Practice, NIPER, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/25/2019</td>
                    <td>Dr. AC Kataky</td>
                    <td>
                      Director and Professor, B. Barooah Cancer Hospital, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>12/6/2019</td>
                    <td>Dr. Atanu Bhattacharjee</td>
                    <td>Associate Professor, ADTU, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>12/6/2019</td>
                    <td>Mrs. Chayanika Bordoloi</td>
                    <td>Assistant Professor, NEF College of Pharmacy, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/28/2020</td>
                    <td>Dr. Sanjay K Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/4/2021</td>
                    <td>Dr. VGM Naidu</td>
                    <td>Dean and Associate Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/15/2022</td>
                    <td>Dr. Dipankar Dakua</td>
                    <td>
                      Palliative Care Physician, Assam Cancer Care Foundation,
                      Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>5/6/2022</td>
                    <td>Dr. Krishna Undela</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>7/12/2022</td>
                    <td>Dr. Krishna Undela</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>9/25/2022</td>
                    <td>Dr. Ramu Adela</td>
                    <td>
                      Assistant Professor, Dept. of Pharmacy Practice, NIPER, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>9/25/2022</td>
                    <td>Dr. Banavath Sandeep Kumar Naik</td>
                    <td>
                      Senior Drug Safety Associate, Accenture Services Limited,
                      Banagalore
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/7/2022</td>
                    <td>Dr. Arun Deka</td>
                    <td>
                      Associate Professor, Dept. of Pain &amp; Palliative Medicine,
                      GMCH
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>11/18/2022</td>
                    <td>Dr. USN Murthy</td>
                    <td>Director, NIPER-G</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/19/2022</td>
                    <td>Dr. T.K. Maity</td>
                    <td>Professor, Jadavpur University, Kolkata</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/20/2022</td>
                    <td>Dr. Dilip Solanki</td>
                    <td>General Manager, Sun Pharma</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/21/2022</td>
                    <td>Dr. Sanjay K Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>2/14/2023</td>
                    <td>Dr. Awanish Mishra</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>3/10/2023</td>
                    <td>Dr. Rajalakshi Devi</td>
                    <td>Professor &amp; HOD, IASST, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>

                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. B.B. Kakoty</td>
                    <td>
                      Professor, Dept. of Pharm. Sciences, Dibrugarh University,
                      Dibrugarh
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. Pankaj Chetia</td>
                    <td>
                      Associate Professor, Dept. of Life Sciences, Dibrugarh
                      University, Dibrugarh
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. Bhanu P. Sahu</td>
                    <td>Professor, Dept. of Pharmacy, GCU, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/22/2023</td>
                    <td>Dr. Subham Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>9/25/2023</td>
                    <td>Mr. Sudhir Chandra Saha</td>
                    <td>Deputy Director Pharmacy &amp; General Secretary, AAPSA</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>9/25/2023</td>
                    <td>Mr. Rafiquz Zaman</td>
                    <td>Chief Advisor, AAPSA</td>
                    <td></td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. (Dr.) T. Puniyamurty</td>
                    <td>Professor, IIT-G</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. (Dr.) Utpal Bora</td>
                    <td>Professor, IIT-G</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Dr. Pijus K. Sasmal</td>
                    <td>Professor, JNU</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. Arup K. Mukharjee</td>
                    <td>Professor, Makaut, WB</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Dr. Bhanu P. Sahu</td>
                    <td>Professor, Dept. of Pharmacy, GCU, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/8/2023</td>
                    <td>Prof. (Dr.) Swati Biswas</td>
                    <td>Professor, BITS Pillani</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Prof. (Dr.) Saorav Pal</td>
                    <td>Professor, Ashoka University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Dr. Sanjay K Banerjee</td>
                    <td>Associate Professor, Dept. of Biotech, NIPER, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Prof. (Dr.) Vidya Avasare)</td>
                    <td>Professor, Ashoka University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Prof. (Dr.) Panna Thapa</td>
                    <td>Professor, Kathmandu University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Dr. K. Gautaman</td>
                    <td>Professor, Govt. College of Pharmacy, Sikkim</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/9/2023</td>
                    <td>Dr. P. Chattopadhyay</td>
                    <td>Professor, DRL, Tezpur</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Prof. (Dr.) Md Abdur Rashid</td>
                    <td>Professor, University of Dhaka</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. B.B. Kakoty</td>
                    <td>
                      Professor, Dept. of Pharm. Sciences, Dibrugarh University,
                      Dibrugarh
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. Pavan M. More</td>
                    <td>ICT, Mumbai</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. S.C. Gupta</td>
                    <td>Professor, AIIMS, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Pro. Mahesh D. Burande</td>
                    <td>Director, IPER, Pune</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Prof. (Dr.) Prodeep Phukan</td>
                    <td>Professor, Gauhati University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>11/10/2023</td>
                    <td>Dr. Bappaditya Gole</td>
                    <td>Professor, Shiv Nadar University</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>1/12/2024</td>
                    <td>Swami Ajatananda Ji</td>
                    <td>RamaKrishna Mission Ashrama, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>2/5/2024</td>
                    <td>Dr. Dipankar Dakua</td>
                    <td>
                      Palliative Care Physician, Assam Cancer Care Foundation,
                      Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>2/29/2024</td>
                    <td>Dr. Ujjal Chakravarty</td>
                    <td>
                      Geeneral and Laproscopic Sergeon, EEN Nursin Home, Guwahati
                    </td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>3/6/2024</td>
                    <td>Prof. (Dr.) Hemanta Kr. Sharma</td>
                    <td>Member, PCI</td>
                    <td>Guest Lecture</td>
                  </tr>

                  <tr>
                    <td>3/6/2024</td>
                    <td>Mrs. Sukriti Das</td>
                    <td>Member, PCI</td>
                    <td></td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Harlokesh N Yadav</td>
                    <td>Additional Professor, AIIMS, New Delhi</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Sumalya Sen</td>
                    <td>
                      Assistant Professor, GD Institute of Medical Sciences and
                      Hospital, Durgapur
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Bhabatosh Das</td>
                    <td>Professor, THSTI, Faridabad</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. S.S. Samajdar</td>
                    <td>Independent Clinical Pharmacologist, Kolkata</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Amit Dey</td>
                    <td>Diabetologist, Apollo Sugar Clinic Ltd.</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Alok Modi</td>
                    <td>Senior Diabetologist, Kevalaya Hostpital, Maharastra</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Praveen TK</td>
                    <td>Professor, JSS College of Pharmacy, Ooty</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Sai Balaji A</td>
                    <td>Scientist/Assistant Professor, CSIR-IICT, Hyderabad</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Ashish Kr. Kakkar</td>
                    <td>Associate Professor, PGIMER, Chandigarh</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Bibyajyoti Saikia</td>
                    <td>Assistant Professor, AIIMS, Guwahati</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Payodhi Dhar</td>
                    <td>Founder &amp; President, Kolkata Nurture Foundation</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Sangeeta Yadav</td>
                    <td>
                      Assistant Professor, Dr. B.S. Ambedkar Medical College, Delhi
                    </td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Parodeep Dwivedi</td>
                    <td>Additional Professor, AIIMS, Jodhpur</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Ashutosh Kumar</td>
                    <td>Assistant Professor, NIPER, SAS Nagar</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Mukesh Nandave</td>
                    <td>Associate Professor, DPSRU, New Delhi</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>09-04-2024 to 10-04-2024</td>
                    <td>Dr. Parkash Diwan</td>
                    <td>Advisor, Indian Pharmacological Society</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/10/2024</td>
                    <td>Mr. Sreejit Soman</td>
                    <td>Application Scientist, Zastra Innovations, Bangalore</td>
                    <td>Resource Person</td>
                  </tr>
                  <tr>
                    <td>5/11/2024</td>
                    <td>Dr. Krishna Undela</td>
                    <td>Assistant Professor, NIPER, Guwahati</td>
                    <td>Guest Lecture</td>
                  </tr>
                  <tr>
                    <td>9/25/2024</td>
                    <td>Mr. Purna Sindhu Mudoi</td>
                    <td>Inspector of Drugs, Kamrup (M)</td>
                    <td>Guest Lecture</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>


        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              2. Departmental Research Statistics
            </h3>
            <div class="text-dark">
              <h3 class="headd3 text-dark fw-bold kd-split-text pt-2">
                <a
                  href="https://www.rgu.ac/research-and-development-rsaps-physics.php"
                >
                  Click More</a
                >
              </h3>
            </div>
          </div>
        </div>


        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              3. The Board of Studies (BoS)
            </h3>
            <div class="table-responsive">
              <table
                class="overflow-hidden table text-wrap table-bordered border-top mb-5"
              >
                <thead class="text-white" style="background-color: #27467a">
                  <tr>
                    <th class="text-white">#</th>
                    <th class="text-white">Position in S-BoS</th>
                    <th class="text-white">Name and Designation</th>
                  </tr>
                </thead>
                <tbody class="para1 align-middle" style="background-color: #f9f9f9; text-align: start;">
                  <tr>
                    <td>1</td>
                    <td>Convener (Ex-Officio)- Head of the Department</td>
                    <td>Prof. L.K. Nath, Dean, RSP</td>
                  </tr>
                  <tr>
                    <td>2</td>
                    <td>All Heads of the Departments - Members (Ex-Officio)</td>
                    <td>Prof. Bipul Nath, Principal HoD, RSP</td>
                  </tr>
                  <tr>
                    <td>3</td>
                    <td>Professors of the School Members (Ex-Officio)</td>
                    <td>
                      Prof. Atanu Bhattacharjee, Professor, RSP
                      <hr />
                      Prof. Prithviraj Chakraborty, Professor, RSP
                    </td>
                  </tr>
                  <tr>
                    <td>4</td>
                    <td>External Experts for each Department (Academic)</td>
                    <td>Prof. Bhanu P Sahu, Dean, GIPS, GCU</td>
                  </tr>
                  <tr>
                    <td>5</td>
                    <td>External Experts for each Department (Industry)</td>
                    <td>Mr. Utpal Kalita, Sr. Manager, Ajanta Pharma Ltd</td>
                  </tr>
                  <tr>
                    <td>6</td>
                    <td>
                      Two Faculty members of the School nominated by the Vice
                      Chancellor
                    </td>
                    <td>
                      Dr. Pranabesh Sikdar, Assistant Professor, RSP
                      <hr />
                      Mr. Kamal Deka, Assistant Professor, RSP
                    </td>
                  </tr>
                  <tr>
                    <td>7</td>
                    <td>Registrar, Member Secretary (Ex-officio)</td>
                    <td>Dr. D N Singh, Registrar or his nominee</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>

        <div>
          <div>
            <h3
              class="headd3 text-dark fw-bold p-3"
              style="background-color: #c7daf9; color: #ffffff"
            >
              4. The Departmental Research Committee (DRC)
            </h3>
            <div class="table-responsive">
              <table
                class="overflow-hidden table text-wrap table-bordered border-top mb-5"
              >
                <thead class="text-white" style="background-color: #27467a">
                  <tr>
                    <th class="text-white">#</th>
                    <th class="text-white">Content</th>
                    <th class="text-white">Name of the Member</th>
                    <th class="text-white">Designation</th>
                    <th class="text-white">Designation in the committee</th>
                  </tr>
                </thead>
                <tbody class="para1 align-middle" style="background-color: #f9f9f9; text-align: start;">
                  <tr>
                    <td>1</td>
                    <td>Head of the Department</td>
                    <td>Prof. (Dr.) Prithviraj Chakraborty</td>
                    <td>Professor&amp; Principal/HoD, RSP</td>
                    <td>Chairman</td>
                  </tr>
                  <tr>
                    <td rowspan="2">2</td>
                    <td rowspan="2" class="align-middle">Two Professors</td>
                    <td>Prof. (Dr.) Bipul Nath</td>
                    <td>Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td>Prof. (Dr.) Atanu Bhattacharjee</td>
                    <td>Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td rowspan="2">3</td>
                    <td rowspan="2" class="align-middle">
                      Two Associate Professors – one of them will be the member
                      secretary
                    </td>
                    <td>Dr. Debarupa Dutta Chakraborty</td>
                    <td>Associate Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td>Dr. Pranabesh Sikdar</td>
                    <td>Associate Professor, RSP</td>
                    <td>Member Secretary</td>
                  </tr>
                  <tr>
                    <td rowspan="2">4</td>
                    <td rowspan="2" class="align-middle">
                      Two Assistant Professors holding Ph.D. degree
                    </td>
                    <td>Dr. Ayesha A.K. Choudhury</td>
                    <td>Assistant Professor, RSP</td>
                    <td>Member</td>
                  </tr>
                  <tr>
                    <td>Dr. Ayesha A.K. Choudhury</td>
                    <td>Dr. Syed Nazrin Ruhina Rahman</td>
                    <td>Assistant Professor, RSP, Member</td>
                  </tr>
                  <tr>
                    <td rowspan="3">5</td>
                    <td rowspan="3" class="align-middle">
                      Three external members not below the rank of Professors,
                      including members from an allied department, who will be
                      nominated by the URC
                    </td>
                    <td>Prof. (Dr.) Mina Ram Nath</td>
                    <td>Professor, Dept. of Botany, RSLSC</td>
                    <td>External Member</td>
                  </tr>
                  <tr>
                    <td>Prof. (Dr.) Ankur Ganguly</td>
                    <td>Professor &amp; Dean, Academics &amp; i/c Dean, RSIT</td>
                    <td>External Member</td>
                  </tr>
                  <tr>
                    <td>Prof. (Dr.) Kaberi Saikia</td>
                    <td>Professor &amp;Principal, RSN.</td>
                    <td>External Member</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>

        <div>
          <h3
            class="headd3 text-dark fw-bold p-3"
            style="background-color: #c7daf9; color: #ffffff"
          >
            5. Course Structure & Syllabus
          </h3>
          <div class="p-2">
            <h3 class="headd3 text-dark fw-bold kd-split-text pt-2">Syllabus -</h3>
            <div class="row">
              <div class="col-lg-12">
                <div class="accordion para1" id="accordionExample">
                  <div class="accordion-item">
                    <h2 class="accordion-header" id="headingOne">
                      <button
                        class="accordion-button collapsed"
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#collapseOne"
                        aria-expanded="false"
                        aria-controls="collapseOne"
                      >
                    <strong>
                      Under Graduate
                      </strong>
                      </button>
                    </h2>
                    <div
                      id="collapseOne"
                      class="accordion-collapse collapse"
                      aria-labelledby="headingOne"
                      data-bs-parent="#accordionExample"
                    >
                      <div class="accordion-body">
                        <div class="accordion-body">
                          <div class="row">
                            <a
                              href="assets/Departmentalpdf/AY 2021-22 D.Pharmacy.pdf"
                              target="_blank"
                            >
                              <i class="fa fa-file-text px-2"></i> AY 2021-22
                              D.Pharmacy.pdf <i class="fa fa-download"></i>
                            </a>
                            <a
                              href="assets/Departmentalpdf/Old Syllabus B. Pharmacy.pdf"
                              target="_blank"
                            >
                              <i class="fa fa-file-text px-2"></i> Old Syllabus B.
                              Pharmacy.pdf <i class="fa fa-download"></i>
                            </a>
                            <a
                              href="assets/Departmentalpdf/Old Syllabus B_Pharm.pdf"
                              target="_blank"
                            >
                              <i class="fa fa-file-text px-2"></i> Old Syllabus B_Pharm.pdf
                              <i class="fa fa-download"></i>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="accordion-item">
                    <h2 class="accordion-header" id="headingTwo">
                      <button
                        class="accordion-button collapsed"
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#collapseTwo"
                        aria-expanded="false"
                        aria-controls="collapseTwo"
                      >
                    <strong>
                      Post Graduate
                      </strong>
                      </button>
                    </h2>
                    <div
                      id="collapseTwo"
                      class="accordion-collapse collapse"
                      aria-labelledby="headingTwo"
                      data-bs-parent="#accordionExample"
                    >
                      <div class="accordion-body"></div>
                    </div>
                  </div>
                  <div class="accordion-item">
                    <h2 class="accordion-header" id="headingThree">
                      <button
                        class="accordion-button collapsed"
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#collapseThree"
                        aria-expanded="false"
                        aria-controls="collapseThree"
                      >
                    <strong>
                      Doctoral Programme
                      </strong>
                      </button>
                    </h2>
                    <div
                      id="collapseThree"
                      class="accordion-collapse collapse"
                      aria-labelledby="headingThree"
                      data-bs-parent="#accordionExample"
                    >
                      <div class="accordion-body">
                        <a
                          href="doctoral-programme.html"
                          class="para1"
                          target="_blank"
                          >
                        <strong>
                        Click to View...
                      </strong>
                      </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-6"></div>
            </div>
          </div>
        </div>
      </div>
          </div>
          </div>
@endsection
